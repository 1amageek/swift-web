...
struct SwiftWebDevSwiftFileClassification: Sendable, Equatable {
    let clientComponentTypeNames: Set<String>
    let hasServerRuntimeSurface: Bool
}

struct SwiftWebDevSwiftFileClassifier: Sendable {
    static func classify(url: URL) throws -> SwiftWebDevSwiftFileClassification {
        let source = try String(contentsOf: url, encoding: .utf8)
        return classify(source: source)
    }

    static func classify(source: String) -> SwiftWebDevSwiftFileClassification {
