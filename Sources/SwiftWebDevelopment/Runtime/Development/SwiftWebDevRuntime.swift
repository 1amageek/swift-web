...
public struct SwiftWebDevRuntime {
    ...
    public func run() throws {
        ...
        let eventLog = SwiftWebDevEventLog(fileURL: SwiftWebDevEventLog.fileURL(for: serverConfiguration))
        try eventLog.reset()
        let devToken = UUID().uuidString
        var server = SwiftWebDevServerProcess(configuration: serverConfiguration, devToken: devToken)
            ...
            do {
                ...
                do {
                    try eventLog.append(SwiftWebDevEvent(
                        kind: .error,
                        message: "Initial Client WASM build failed for \(runtime.componentTypeName): \(String(describing: error))"
                    ))
                } catch {
                    logger.error(
    ...
    private func handle(
        ...
        for runtime in plan.clientRuntimes {
            do {
                ...
                try eventLog.append(SwiftWebDevEvent(
                    kind: .error,
                    message: "Client WASM HMR failed for \(runtime.componentTypeName): \(String(describing: error))",
                    changedPaths: plan.changedPaths
                ))
                logger.error(
        ...
        server.stop()
        logger.info("SwiftWeb dev child process restarting", metadata: metadata())
        do {
            try server.start()
        } catch {
            try eventLog.append(SwiftWebDevEvent(
            ...
            logger.error(
                "SwiftWeb dev child process failed to restart",
                metadata: metadata([
                    "error": .string(String(describing: error)),
                ])
            )
