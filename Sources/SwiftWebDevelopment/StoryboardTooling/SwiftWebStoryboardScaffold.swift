...
public enum SwiftWebStoryboardScaffoldError: Error, Sendable, CustomStringConvertible {
    ...
    public var description: String {
        ...
        case .unmanagedDirectory(let directory):
            "storyboard directory already exists and is not managed by sweb: \(directory.path)"
...
///
/// The catalog UI lives in the independent `Storyboard` package's application
...
public struct SwiftWebStoryboardScaffold: Sendable {
    ...
    /// The generated app's module name. It preserves the legacy preview module identity.
    /// The independent authored app uses the SwiftWebStoryboard module instead.
    ...
    /// Links catalog and route sources from the independent Storyboard package,
    /// preserving their layout. The preview owns its own App.swift entry point.
    private func linkCatalogSources() throws {
        ...
        let sources = try swiftFiles(in: catalogSourceDirectory, relativePath: "")
            .filter { $0.relativePath != "App.swift" }
    ...
    private func swiftHTMLDependencyDeclaration() throws -> String {
        ...
        return #".package(url: "https://github.com/1amageek/swift-html.git", from: "0.16.1")"#
    }
    ...
    private var readme: String {
        ...
        This package is generated by `sweb storyboard`.

        It hosts the SwiftWebUI component storyboard for inspecting component styles
        across color schemes and themes. The catalog itself lives in the repository's independent
        `Storyboard` package; this package is a thin app that links those
        sources so the dev runtime can build the client WASM runtime. The app source
        package is not modified.

        ```bash
        sweb storyboard --package-path \(Self.shellPath(projectDirectory.deletingLastPathComponent().deletingLastPathComponent().path))
        ```
        """
    }
