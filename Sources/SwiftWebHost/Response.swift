...
public struct Response: Sendable {
    public struct Body: Sendable {
        enum Storage: Sendable {
            case none
            case bytes([UInt8])
            case stream(@Sendable (any BodyWriter) async throws -> Void)
        }
        ...
        public init(managedAsyncStream: @escaping @Sendable (any BodyWriter) async throws -> Void) {
        ...
        public var stream: (@Sendable (any BodyWriter) async throws -> Void)? {
            switch storage {
            case .none, .bytes:
                nil
    ...
    public mutating func setCookie(_ name: String, _ value: CookieValue) {
