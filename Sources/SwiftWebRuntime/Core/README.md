...
The SwiftWeb component API is documented in [`docs/ActorInjectionDesign.md`](../../../docs/ActorInjectionDesign.md). A client component should read an `@RemoteActor` property as the resolved service object; `WebActorSystem`, actor ids, and `$Protocol.resolve(id:using:)` remain runtime details for the standard API.
...
| API shape | Client components use `@RemoteActor` as the resolved service object and call `distributed func` as if it were local. |
