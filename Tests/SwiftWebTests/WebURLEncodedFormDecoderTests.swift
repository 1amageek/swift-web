...
@Suite
struct WebURLEncodedFormDecoderTests {
    ...
    func decodesScalarsOptionalsAndArrays() throws {
        let decoder = WebURLEncodedFormDecoder()
    ...
    func absentOptionalsDecodeAsNil() throws {
        let decoded = try WebURLEncodedFormDecoder().decode(SearchParams.self, from: "q=x&page=1")
    ...
    func missingRequiredKeyThrows() {
        #expect(throws: DecodingError.self) {
            try WebURLEncodedFormDecoder().decode(SearchParams.self, from: "page=1")
        }
    }
    ...
    func percentDecodesNamesAndValues() throws {
        ...
        let decoded = try WebURLEncodedFormDecoder().decode(
            Form.self,
