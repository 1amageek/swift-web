...
struct SwiftPMWasmArtifactTests {
  ...
  @Test
  func resolvesExplicitProductArtifactNameBeforeTargetFallbacks() throws {
    let root = FileManager.default.temporaryDirectory
      .appendingPathComponent("SwiftPMWasmArtifactTests-\(UUID().uuidString)", isDirectory: true)
    defer {
      do {
    ...
    let targetArtifact =
      scratch
      .appendingPathComponent("wasm32-unknown-wasip1/release/StoryboardPreviewWasmRuntime.wasm")
    ...
    try write("public struct Runtime {}", to: source)
    try write("product", to: productArtifact)
    try write("target", to: targetArtifact)
    ...
    #expect(resolvedURL.standardizedFileURL == productArtifact.standardizedFileURL)
  }
