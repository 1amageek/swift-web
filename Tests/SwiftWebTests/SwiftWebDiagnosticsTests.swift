...
struct SwiftWebDiagnosticsTests {
  ...
  func wasmRuntimeHostScriptUsesWasmExportsAndManifestLoading() {
    ...
    #expect(source.contains("while (status === 2)"))
    ...
    #expect(source.contains("await this.bootstrapBundle(this.primaryBundleID"))
    #expect(source.contains("await this.ensureRuntimeStarted(instance)"))
    #expect(source.contains("isStarted: false"))
    #expect(source.contains("startPromise: null"))
    #expect(source.contains("installServerActionListeners()"))
    #expect(source.contains("installNavigationListeners()"))
    #expect(source.contains("navigationAnchorForEvent(event)"))
    #expect(source.contains("navigateServerDocument(target.href, { history: \"push\" })"))
    #expect(source.contains("window.addEventListener(\"popstate\""))
    #expect(source.contains("currentDocumentURL"))
    #expect(source.contains("isSameDocumentURL(target)"))
    #expect(source.contains("\"X-SwiftWeb-Navigation\": \"client\""))
    #expect(source.contains("window.history.pushState"))
    #expect(source.contains("rebootstrapServerDocument(\"navigation\")"))
    ...
    #expect(source.contains("appendFormDataToURL(formData, url)"))
    ...
    #expect(source.contains("let boundary = target.closest(\"[data-component]\")"))
    #expect(source.contains("(record) => rawValue(record.componentID) === componentID"))
    #expect(source.contains("if (resolved.bundleID !== this.primaryBundleID)"))
    #expect(source.contains("boundary.parentElement.closest(\"[data-component]\")"))
    #expect(source.contains("response.appliesDOMCommandsInRuntime !== true"))
    #expect(source.contains("const body = method === \"GET\" ? null : urlEncodedFormBody(formData)"))
    #expect(source.contains("const body = new URLSearchParams()"))
    #expect(source.contains("SwiftWeb server actions do not support file field"))
    #expect(source.contains("callRuntime(exportName, payload, instance = this.primaryInstance)"))
    #expect(source.contains("const previousState = this.captureHotUpdateState()"))
    #expect(source.contains("const previousDOMState = this.captureHotUpdateDOMState()"))
    #expect(source.contains("this.restoreHotUpdateDOMState(previousDOMState)"))
    #expect(source.contains("document.documentElement.cloneNode(false)"))
    #expect(source.contains("document.head.cloneNode(true)"))
    #expect(source.contains("copyElementAttributes(document.documentElement, state.documentElement)"))
    #expect(source.contains("this.restoreHotUpdateState(previousState)"))
    #expect(!source.contains("hotUpdateQueue"))
    #expect(source.contains("this.hotUpdateContentHashes = new Map()"))
    #expect(source.contains("const operation = this.eventQueue"))
    #expect(source.contains("return this.applyHotUpdateBatchNow(updates);"))
    #expect(source.contains("this.eventQueue = operation.catch(() => {});"))
    #expect(source.contains("hmr.clientComponent.cacheHit"))
    #expect(source.contains("hmr.clientRuntimeBatch.failed"))
    #expect(source.contains("applyCommandBatch(result.response.commandBatch, this, { strict: true })"))
    #expect(source.contains("SwiftWeb DOM command could not be applied"))
  ...
  func devHotReloadInjectsReloadWaitScriptAndTokenHeaderWhenEnabled() {
    ...
    let clientScript = SwiftWebDevHotReload.clientScript()

    #expect(html.contains("<main>Counter</main><script>"))
    #expect(html.contains("token: \"reload-token\""))
    #expect(html.contains("reloadPath: \"/__swiftweb/dev/reload\""))
    #expect(html.contains("eventsPath: \"/__swiftweb/dev/events\""))
    #expect(html.contains("src=\"/__swiftweb/dev/client.js?v="))
    #expect(!html.contains("fetch(swiftWebDevReloadURL.href"))
    #expect(!html.contains("devStatus"))
    #expect(clientScript.contains("fetch(swiftWebDevReloadURL.href"))
    #expect(clientScript.contains("__swiftWebDevReload"))
    #expect(clientScript.contains("closed: false"))
    #expect(clientScript.contains(#"window.addEventListener("pagehide", swiftWebHandlePageHide)"#))
    #expect(clientScript.contains(#"window.removeEventListener("pagehide", swiftWebHandlePageHide)"#))
    #expect(clientScript.contains("if (!event.persisted)"))
    #expect(clientScript.contains("devStatus"))
    #expect(clientScript.contains("clientBuildStarted"))
    #expect(clientScript.contains("lastErrorEvent"))
    #expect(clientScript.contains("lastServerRebuildErrorEvent"))
    #expect(clientScript.contains(#""swiftWebError""#))
    #expect(clientScript.contains("wasmStatus"))
    #expect(clientScript.contains("SwiftWeb dev session changed"))
    #expect(clientScript.contains("if (!swiftWebStartEventStream() && !swiftWebStartFetchEventStream())"))
    #expect(clientScript.contains("source.readyState === EventSource.CONNECTING"))
    #expect(clientScript.contains("didProbeClosedSource = true"))
    #expect(!clientScript.contains("}, 1200)"))
    #expect(clientScript.contains("SwiftWeb HMR stream failed"))
    #expect(clientScript.contains("SwiftWeb HMR event failed"))
    #expect(clientScript.contains("Client WASM failed"))
    #expect(clientScript.contains("phase === \"error\" || phase === \"failed\""))
    #expect(!clientScript.contains("reload-token"))
    #expect(!clientScript.contains("setInterval"))
    #expect(headers[SwiftWebDevHotReload.reloadTokenHeaderName] == "reload-token")
    #expect(headers[.cacheControl] == "no-cache")
    #expect(SwiftWebDevHotReload.clientScriptHeaders()[.cacheControl] == "public, max-age=31536000, immutable")
  }
}
