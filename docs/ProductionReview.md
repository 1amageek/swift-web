...
| Build child process | `Mutex<State>` cancellation owner storing the process, verified process-group ID, and owner-lifetime monitor | Host-only | Not constructed | The monitor snapshots the complete descendant tree, including compiler subprocesses moved into independent process groups. Timeout, cancellation, normal leader exit, and abrupt owner exit drain tracked descendants with `SIGTERM`, then `SIGKILL` after the configured grace period; cancellation is rechecked after that drain | Bounded tree and process-group exit waits precede controller release |
...
`swift-6.4.x-DEVELOPMENT-SNAPSHOT-2026-07-23-a_wasm` SDK.
